...
sort(vcat(collect(Kfold(length(y), 5))...))
Juno.@enter coef(hurdleglrfit;select=MinCVmse(Kfold(length(y), 5)))
coefsJCVmin=vcat(coef(hurdleglrfit;select=MinCVmse(Kfold(length(y), 5)))...)
